...
def _summary_txt(payload: dict) -> str:
    lines = [
        ...
        f"Pack ready: {bool(payload.get('pack_ready'))}",
...
class UtilityModelProofReviewService:
    def __init__(self, config: AppConfig, review_packs: ReviewPackService):
        self.config = config
        self.review_packs = review_packs
        self.proof_root = ensure_dir(Path(config.model_dir) / 'utility_model_proof')
        self.root_dir = ensure_dir(Path(config.model_dir) / 'utility_model_proof_review')
        self.summary_path = self.root_dir / 'latest_utility_model_proof_review_summary.json'
        self.pack_path = self.root_dir / 'latest_utility_model_proof_review_pack.zip'
        self.session_path = self.proof_root / 'latest_utility_model_proof_session.json'
        self.proof_summary_path = self.proof_root / 'latest_utility_model_proof_summary.json'
    ...
    def latest_pack(self) -> Path | None:
        ...
        if not bool(summary.get('pack_ready')):
            return None
    ...
    def _matching_runs(self, session: dict) -> list[dict]:
        ...
        for row in rows:
            ...
            proof = dict(status.get('live_utility_model_proof') or {})
            if not proof:
                continue
            if target_session and str(proof.get('proof_session_id') or '').strip() != target_session:
                continue
            item['live_utility_model_proof'] = proof
            out.append(item)
        return out
    ...
    def build_summary(self, *, reason: str = 'refresh') -> dict:
        ...
        if not session:
            payload = {
                ...
                'headline': 'No retained utility-model proof session is available',
                'status': 'waiting',
                'pack_ready': False,
                'verdict': 'no_utility_model_proof_session',
                'recommended_action': 'activate_a_controlled_utility_model_proof_first',
                'summary': 'There is no retained utility-model proof session on disk, so no isolated proof-window evidence can be reviewed yet.',
                'reason': reason,
            }
        ...
        headline = 'Controlled utility-model proof is waiting for evidence'
        status = 'collecting'
        pack_ready = False
        verdict = 'waiting_for_utility_model_proof_evidence'
        recommended_action = 'keep_collecting_isolated_evidence_for_the_utility_model_candidate'
        summary = 'The proof window for the utility-model challenger has not yet matured enough for a decision-grade verdict.'
        visible_n = len(visible_rows)
        utility_score = _f(utility.get('scan_shortlist_utility_score'))
        if not runs:
            headline = 'No matching utility-model proof-window runs have been captured yet'
            status = 'waiting'
            pack_ready = False
            verdict = 'no_matching_utility_model_proof_runs'
            recommended_action = 'wait_for_scans_under_the_utility_model_candidate'
            summary = 'The utility-model proof session exists, but no matching attributed runs have been recorded yet.'
        elif visible_n < 30:
            headline = 'Controlled utility-model proof is collecting evidence'
            status = 'collecting'
            pack_ready = False
            verdict = 'waiting_for_more_resolved_visible_rows'
            recommended_action = 'allow_more_utility_model_runs_to_mature'
            ...
            headline = 'Controlled utility-model proof currently supports the challenger'
            status = 'ready'
            pack_ready = True
            verdict = 'utility_model_proof_supports_candidate'
            recommended_action = 'prepare_a_controlled_adoption_gate_for_the_utility_model_candidate'
            summary = 'The utility-model challenger is beating the hidden remainder in isolated live evidence.'
        elif gap is not None and gap < 0 and (utility_score is None or utility_score < 0):
            headline = 'Controlled utility-model proof currently rejects the challenger'
            status = 'ready'
            pack_ready = True
            verdict = 'utility_model_proof_rejects_candidate'
            recommended_action = 'clear_the_utility_model_proof_and_keep_current_live_model'
            summary = 'The utility-model challenger is not beating the hidden remainder in isolated live evidence.'
        else:
            headline = 'Controlled utility-model proof remains mixed'
            status = 'ready'
            pack_ready = True
            verdict = 'utility_model_proof_inconclusive'
            recommended_action = 'continue_the_proof_or_keep_current_live_model'
            summary = 'The utility-model challenger has some isolated live evidence, but it is not decisive yet.'
        payload = {
            'available': True,
            'generated_at_utc': _utc_now_iso(),
            'app_version': APP_VERSION,
            ...
            'decision_memo_markdown': (
                '# Controlled utility-model proof review\n\n'
                f'- **Headline:** {headline}\n'
                f'- **Verdict:** {verdict}\n'
                f'- **Recommended action:** {recommended_action}\n\n'
                '## Why this tranche exists\n'
                '- The utility model lab can produce a stronger offline challenger.\n'
                '- The next missing step is isolated live proof for that exact model.\n'
                '- This review decides whether the utility-model challenger deserves a real adoption gate.\n'
            ),
        }
        atomic_write_json(self.summary_path, payload)
    ...
    def _build_pack(self, summary: dict, session: dict, runs: list[dict], evaluated_runs: list[dict]):
        with zipfile.ZipFile(self.pack_path, 'w', compression=zipfile.ZIP_DEFLATED) as zf:
            zf.writestr('utility_model_proof_review_summary.json', _json_bytes(summary))
            zf.writestr('utility_model_proof_review_summary.txt', _summary_txt(summary).encode('utf-8'))
            zf.writestr('utility_model_proof_session.json', _json_bytes(session or {}))
            zf.writestr('matching_utility_model_proof_runs.json', _json_bytes(runs or []))
            zf.writestr('evaluated_utility_model_proof_runs.json', _json_bytes(evaluated_runs or []))
