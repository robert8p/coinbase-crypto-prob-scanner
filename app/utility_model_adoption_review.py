...
class UtilityModelAdoptionReviewService:
    def __init__(self, config: AppConfig, review_packs: ReviewPackService):
        self.config = config
        self.review_packs = review_packs
        self.adoption_root = ensure_dir(Path(config.model_dir) / 'utility_model_adoption')
        self.root_dir = ensure_dir(Path(config.model_dir) / 'utility_model_adoption_review')
        self.summary_path = self.root_dir / 'latest_utility_model_adoption_review_summary.json'
        self.pack_path = self.root_dir / 'latest_utility_model_adoption_review_pack.zip'
        self.state_path = self.adoption_root / 'latest_utility_model_adoption_state.json'
        self.adoption_summary_path = self.adoption_root / 'latest_utility_model_adoption_summary.json'
    ...
    def _matching_runs(self, session: dict) -> list[dict]:
        ...
        for row in rows:
            ...
            adoption = dict(status.get('live_utility_model_adoption') or {})
            if not adoption:
                continue
            if target_session and str(adoption.get('adoption_session_id') or '').strip() != target_session:
                continue
            item['live_utility_model_adoption'] = adoption
            out.append(item)
        return out
    ...
    def build_summary(self, *, reason: str = 'refresh') -> dict:
        ...
        if not session:
            payload = {
                ...
                'headline': 'No retained utility-model adoption session is available',
                'verdict': 'no_utility_model_adoption_session',
                'recommended_action': 'activate_a_controlled_utility_model_adoption_first',
                'summary': 'There is no retained utility-model adoption session on disk, so no isolated adopted-path evidence can be reviewed yet.',
                'reason': reason,
            }
        ...
        headline = 'Controlled utility-model adoption review is waiting for evidence'
        verdict = 'waiting_for_utility_model_adoption_evidence'
        recommended_action = 'keep_collecting_isolated_adoption_scope_evidence'
        summary = 'The adopted utility-model path has not yet matured enough for a keep-versus-rollback verdict.'
        ...
        if not runs:
            verdict = 'no_matching_utility_model_adoption_runs'
            headline = 'No matching utility-model adoption runs are available yet'
            recommended_action = 'wait_for_scans_under_the_adopted_utility_model'
            summary = 'The controlled utility-model adoption exists, but no matching review-pack runs have been resolved for it yet.'
        elif visible_n < 40:
            verdict = 'waiting_for_more_resolved_visible_rows'
            headline = 'Utility-model adoption review needs more resolved visible rows'
            recommended_action = 'keep_collecting_isolated_adoption_scope_evidence'
            summary = 'The adopted utility-model path has some evidence, but not enough resolved visible rows for a decision-grade keep-versus-rollback verdict.'
        elif gap is not None and gap < 0 and (gap_delta is None or gap_delta <= 0):
            verdict = 'utility_model_adoption_review_recommends_rollback'
            headline = 'Utility-model adoption review recommends rollback'
            recommended_action = 'clear_the_controlled_utility_model_adoption'
            summary = 'The isolated adopted utility-model path is underperforming the hidden remainder and does not beat the activation baseline.'
        elif utility_score is not None and utility_score > 0 and gap is not None and gap > 0 and (gap_delta is None or gap_delta >= 0):
            verdict = 'utility_model_adoption_review_supports_keeping_candidate'
            headline = 'Utility-model adoption review supports keeping the adopted candidate'
            recommended_action = 'keep_the_adopted_utility_model_and_continue_monitoring'
            summary = 'The isolated adopted utility-model path continues to beat the hidden remainder and is at least as strong as the activation baseline.'
        else:
            verdict = 'utility_model_adoption_review_inconclusive'
            headline = 'Utility-model adoption review is mixed'
            recommended_action = 'continue_monitoring_or_rollback_if_operator_risk_tolerance_is_low'
            summary = 'The adopted utility-model path is producing evidence, but it is not yet decisive enough to lock in a keep-versus-rollback conclusion.'
        payload = {
            'available': True,
            'generated_at_utc': _utc_now_iso(),
            'app_version': APP_VERSION,
            ...
            'summary': summary,
            'reason': reason,
            'candidate_model': dict(adoption_summary.get('candidate_model') or {}),
            'utility_params': dict(adoption_summary.get('utility_params') or {}),
            ...
            'decision_memo_markdown': (
                '# Controlled utility-model adoption review\n\n'
                f'- **Headline:** {headline}\n'
                f'- **Verdict:** {verdict}\n'
                f'- **Recommended action:** {recommended_action}\n\n'
                '## Why this tranche exists\n'
                '- The adoption gate can activate a rollback-aware adopted utility-model path.\n'
    ...
    def _build_pack(self, summary: dict, session: dict, runs: list[dict], evaluated_runs: list[dict], adoption_summary: dict):
        with zipfile.ZipFile(self.pack_path, 'w', compression=zipfile.ZIP_DEFLATED) as zf:
            zf.writestr('utility_model_adoption_review_summary.json', _json_bytes(summary))
            zf.writestr('utility_model_adoption_review_summary.txt', _summary_txt(summary).encode('utf-8'))
            zf.writestr('utility_model_adoption_session.json', _json_bytes(session or {}))
            zf.writestr('utility_model_adoption_summary.json', _json_bytes(adoption_summary or {}))
            zf.writestr('matching_utility_model_adoption_runs.json', _json_bytes(runs or []))
            zf.writestr('evaluated_utility_model_adoption_runs.json', _json_bytes(evaluated_runs or []))
